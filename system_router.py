...
def get_system_status(supabase_client: Client) -> str:
    ...
    if supabase_client:
        try:
            res_art = supabase_client.from_("newspaper_articles").select("id", count="exact").limit(1).execute()
            total_articles = res_art.count or 0

            res_num = supabase_client.from_("newspaper_numbers").select("id", count="exact").limit(1).execute()
            total_numbers = res_num.count or 0

            res_date = supabase_client.from_("newspaper_issues").select("publication_date").order("publication_date", desc=True).limit(1).execute()
            if res_date.data:
                latest_date = res_date.data[0].get("publication_date", "မသိရှိပါ")
        except Exception as e:
            logger.warning("Status query warning: %s", e)
...
"""
    try:
        res = genai_client.models.generate_content(model=gemini_model, contents=prompt)
